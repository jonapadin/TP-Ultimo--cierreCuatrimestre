...
document.getElementById("comprar").addEventListener("click", () => {
  ...
  if (total > 0) {
    check.classList.remove("rechaza");
    ...
    for (let i = 0; i < productos.length; i++) {
      if (
        ...
        cantidadComprada[i] <= productos[i].stock
      ) {
        productos[i].stock -= cantidadComprada[i];
        document.getElementById(
          `stock-${i}`
        ).innerText = `Stock actual: ${productos[i].stock}`;
      }
    }
  } else {
    check.classList.remove("confirma");
    check.classList.add("rechaza");
    check.innerText = "No hay productos seleccionados, no hay stock o la cantidad es cero.";
  }
});
